...
async function confirmDelete() {
  ...
  deleting.value = true
  const isBackendCourse = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(
    target.id,
  )
  try {
    if (isBackendCourse) {
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex items-center gap-1 rounded-full bg-gray-100 p-1">
        <button
          ...
          :class="statusFilter === tab.value ? 'bg-white text-text shadow-sm' : 'text-text-soft'"
    ...
    <BaseCard class="overflow-hidden !p-0">
      <div class="overflow-x-auto">
        <table class="w-full min-w-[720px] text-sm">
          <thead>
            <tr
              class="border-b border-border text-left text-xs font-semibold uppercase tracking-wider text-text-soft"
            >
              ...
              <td class="px-5 py-4">
                <div class="flex items-center justify-end gap-1">
                  <RouterLink
                    ...
                    class="rounded-md p-2 text-text-soft transition-colors hover:bg-yellow-100 hover:text-yellow-600"
